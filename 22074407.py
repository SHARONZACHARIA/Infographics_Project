...
plt.subplot(3, 2, 3)
pivot_df_Hindex.plot(kind='bar', width=0.8, ax=plt.gca(), color=[get_country_color(country) for country in pivot_df_Hindex.columns])
...
plt.subplot(3, 2, 4)
pivot_df_CPI.plot(kind="barh", ax=plt.gca(), color=[get_country_color(country) for country in pivot_df_CPI.columns])
...
plt.pie(
    ...
    autopct='%1.1f%%',
    startangle=140,
    colors=[get_country_color(country) for country in df_filtered_dystopia["Country"]],
    explode=explode,
    shadow=True,
)
...
pivot_df_Freedom.plot(kind="line", ax=plt.gca(), color=[get_country_color(country) for country in pivot_df_Freedom.columns])
plt.legend().remove()  # Remove individual legend

# Create a single legend and adjust its position using bbox_to_anchor
handles = [plt.Line2D([0], [0], color=get_country_color(country), marker='o', markersize=5, linestyle='', label=country) for country in COUNTRY_NAMES]
legend = plt.figlegend(handles=handles, labels=COUNTRY_NAMES, loc='center', ncol=5, fontsize=8, bbox_to_anchor=(0.5, 0.35), bbox_transform=plt.gcf().transFigure)
...
# Save the plot
plt.savefig('combined_plots.png', dpi=300, bbox_extra_artists=(legend,), bbox_inches='tight')
# Show the plots
plt.show()